...
use dcp_types::{AutomationCommand, AutomationResult, KeyModifier, MouseButton};
    ...
    async fn execute(
        ...
        let result = match command {
            ...
            AutomationCommand::MouseScroll {
                ...
                .await
                .map(|_| "scrolled"),
            AutomationCommand::KeyboardType { text } => self.type_text(text).await.map(|_| "typed"),
            ...
            AutomationCommand::ClipboardSet {
                content,
                content_type,
            } => self
                .set_clipboard(content, content_type.as_deref())
                .await
                .map(|_| "clipboard set"),
            AutomationCommand::AppLaunch {
                executable,
                args,
                working_dir,
            } => self
                .launch_app(executable, args, working_dir.as_deref())
                .await
                .map(|_| "app launched"),
            AutomationCommand::WindowFocus { window_id } => self
                .focus_window(*window_id)
                .await
                .map(|_| "window focused"),
            AutomationCommand::WindowMove { window_id, x, y } => self
            ...
            AutomationCommand::WindowResize {
                ...
                .await
                .map(|_| "window resized"),
            AutomationCommand::WindowMinimize { window_id } => self
                .minimize_window(*window_id)
                .await
                .map(|_| "window minimized"),
            AutomationCommand::WindowMaximize { window_id } => self
                .maximize_window(*window_id)
                .await
                .map(|_| "window maximized"),
            AutomationCommand::WindowRestore { window_id } => self
                .restore_window(*window_id)
                .await
                .map(|_| "window restored"),
